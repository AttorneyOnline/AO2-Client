...
void Courtroom::list_areas()
{
  ...
  for (int n_area = 0; n_area < area_list.size(); ++n_area) {
    ...
    // Coloring logic here.
    // TODO: This whole area status system is unnecessarily hardcoded
...
// EMOTE MOD OVERRIDES:
  ...
  // Finally, we send over if we want our text to process immediately, instead of after the preanimation.
  ...
  for (const QString &f_effect : effects_to_check) {
    QString packet;
...
void Courtroom::do_transition(QString p_desk_mod, QString old_pos, QString new_pos) {
    ...
    else if (m_chatmessage[EMOTE_MOD].toInt() != IDLE)
...
#endif
>>>>>>> 7c1fe80d4cbece55149b6fd52e3e90515f440446
}

...
void Courtroom::on_ooc_return_pressed() {
  ...
  if (ooc_message.startsWith("/pos")) {
    ...
    return;
             ...
             ooc_message.startsWith("/save_ooc")) {
    bool is_ooc = !ooc_message.startsWith("/save_ic");
    save_log_result result = save_chatlog(is_ooc);
    switch (result) {
    case ERR_UNKNOWN:
      append_server_chatmessage(
          "CLIENT", tr("An unknown error occurred while saving the chatlog."),
          "1");
      ui_ooc_chat_message->clear();
      break;
    case ERR_CREATELOG:
      append_server_chatmessage(
          "CLIENT",
          tr("Couldn't create the log file, please check folder permissions."),
          "1");
      ui_ooc_chat_message->clear();
      break;
    case ERR_CREATEDIR:
      append_server_chatmessage("CLIENT",
                                tr("Unable to create log directory, please "
    ...
    case SUCCESS:
      append_server_chatmessage(
          "CLIENT", tr("The requested chatlog has been saved."), "1");
      ui_ooc_chat_message->clear();
      break;
    }
    return;

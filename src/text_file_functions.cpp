...
bool AOApplication::get_slower_blips() {
  QString result = configini->value("slower_blips", "false").value<QString>();
...
AOApplication::format_ooc AOApplication::get_saved_ooc_format() {
  QString value_string =
      configini->value("saved_ooc_format", "Plain text (default)")
          .value<QString>();
...
Courtroom::save_log_result Courtroom::save_chatlog(bool is_ooc) {
  ...
  QString dtstamp_clean =
      QDateTime::currentDateTimeUtc()
          .toString()
          .replace(" ", "-")
          .replace(":", ".")
          .replace("GMT", ""); // colons are illegal in Windows filenames, and
                               // spaces are for chumps
  ...
  out << "#####" << '\n'
      << "Saved " + log_type + " history from " << ao_app->current_server_name
      << " (" << ao_app->current_server_address << ") on "

    ...
    , ao_app(ao_app)
{
  ...
  ui_character = new kal::CharacterAnimationLayer(ao_app, ui_viewport);
  ui_character->setObjectName("ui_vp_player_char");
...
void AOEmotePreview::updateViewportGeometry()
{
  ...
  ui_character->move(0, 0);
  ui_character->resize(ui_viewport->width(), ui_viewport->height());
...
void AOEmotePreview::display(QString character, QString emote, kal::CharacterAnimationLayer::EmoteType emoteType, bool flipped, int xOffset, int yOffset)
{
  ...
  m_emote = emote;
  ui_character->stopPlayback();
  ui_character->move(ui_viewport->width() * xOffset / 100, ui_viewport->height() * yOffset / 100);
  ui_character->loadCharacterEmote(character, emote, emoteType);
  ui_character->setPlayOnce(false);
  ui_character->setFlipped(flipped);
  ui_character->startPlayback();

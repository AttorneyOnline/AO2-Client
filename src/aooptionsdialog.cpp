...
AOOptionsDialog::AOOptionsDialog(QWidget *parent, AOApplication *p_ao_app)
  ...
  ui_slower_blips_lb->setToolTip(
      tr("Set the text speed to be the same as the AA games."));
  ...
  ui_pun_delay->setToolTip(
      tr("Punctuation delay modifier."
  ...
  ui_settings_tabs->addTab(ui_other_tab, tr("Chatlogs"));
  ...
  ui_ooc_format_label = new QLabel(ui_other_widget);
  ui_ooc_format_label->setText(tr("Saved OOC Log Format:"));
  ui_ooc_format_label->setToolTip(
      tr("Sets the formatting used when saving the OOC log via /save_ooc. If "
         "you're not sure, just use plain text."));
  ui_other_layout->setWidget(6, QFormLayout::LabelRole, ui_ooc_format_label);

  ui_ooc_format_combobox = new QComboBox(ui_other_widget);
  ...
  ui_other_layout->setWidget(6, QFormLayout::FieldRole, ui_ooc_format_combobox);
...
void AOOptionsDialog::save_pressed() {
  ...
  configini->setValue("saved_ooc_format",
                      ui_ooc_format_combobox->currentText());

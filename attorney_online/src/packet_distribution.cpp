...
void AOApplication::append_to_demofile(QString packet_string)
{
  if (options.logToDemoFileEnabled() && !log_filename.isEmpty()) {
    QString path =
        log_filename.left(log_filename.size()).replace(".log", ".demo");
    if (!demo_timer.isValid())
      demo_timer.start();
    else
      append_to_file("wait#" + QString::number(demo_timer.restart()) + "#%",
                     path, true);
...
void AOApplication::server_packet_received(AOPacket *p_packet)
{
...
#ifdef DEBUG_NETWORK
  if (header != "checkconnection") qDebug() << "R:" << f_packet;
#endif

  if (header == "decryptor") {
    if (f_contents.size() == 0) goto end;
    ...
    if (f_packet.contains("arup", Qt::CaseInsensitive)) arup_supported = true;
    ...
    if (!lobby_constructed || f_contents.size() < 2) goto end;
    ...
    if (courtroom_constructed) w_courtroom->set_window_title(window_title);
    ...
    if (options.logToDemoFileEnabled() && server_name != "Demo playback") {
      ...
      this->write_to_file("Joined server " + server_name +
                              " hosted on address " + server_address + " on " +
    ...
    if (!courtroom_constructed) goto end;
    ...
    for (int n_element = 0; n_element < f_contents.size(); ++n_element) {
      ...
      if (sub_elements.size() >= 2) f_char.description = sub_elements.at(1);
    ...
    if (!courtroom_constructed || courtroom_loaded) goto end;
  ...
  else if (header == "FA") // Fetch areas ONLY
  {
    ...
    if (!courtroom_constructed || f_contents.isEmpty()) goto end;
  ...
  // server accepting char request(CC) packet
    ...
    if (!courtroom_constructed || f_contents.size() < 3) goto end;
    // For some reason, args 0 and 1 are not used (from tsu3 they're client ID
    // and a string "CID")
    ...
    if (f_contents.isEmpty()) goto end;
    ...
    if (courtroom_constructed) {
      ...
      for (QString f_string : f_contents_encoded) {
        ...
        if (sub_contents.size() < 3) continue;
    ...
    if (!courtroom_constructed || f_contents.size() < 2) goto end;
    ...
    if (ping_time != -1) latency = ping_time;
    log_to_demo = false;
  }
  // Subtheme packet
  else if (header == "ST") {
    ...
    if (p_st.toLower() != "server")
      // We don't. Simply acknowledge the subtheme sent by the server, but don't
      // do anything else.
      return;
  ...
  // Auth packet
    ...
    if (!courtroom_constructed || !auth_packet_supported ||
        f_contents.isEmpty()) {
      goto end;
    }
    bool ok;
    ...
    bool ok;
    Courtroom::JudgeState state =
        static_cast<Courtroom::JudgeState>(f_contents.at(0).toInt(&ok));
    if (!ok) {
      goto end; // ignore malformed packet
    }
    w_courtroom->set_judge_state(state);
    if (w_courtroom->get_judge_state() !=
        Courtroom::POS_DEPENDENT) { // If we receive JD -1, it means the server
                                    // asks us to fall back to client-side judge
                                    // buttons behavior
      w_courtroom->show_judge_controls(w_courtroom->get_judge_state() ==
                                       Courtroom::SHOW_CONTROLS);
    }
    else {
  ...
  // AssetURL Packet
    ...
    if (f_contents.size() > 1 ||
        f_contents.isEmpty()) { // This can never be more than one link.
      goto end;
    }
    QUrl t_asset_url = QUrl::fromPercentEncoding(f_contents.at(0).toUtf8());
    if (t_asset_url.isValid()) asset_url = t_asset_url.toString();
  }

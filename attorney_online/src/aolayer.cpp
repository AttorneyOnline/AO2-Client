                                   ...
                                   QString p_miscname)
{
  ...
  QString final_image = ao_app->get_image(p_filename, options.theme(), options.subTheme(), ao_app->default_theme, p_miscname, p_charname, "placeholder");
...
void InterfaceLayer::load_image(QString p_filename, QString p_miscname)
{
  last_path = "";
  stretch = true;
  QString final_image = ao_app->get_image(p_filename, options.theme(), options.subTheme(), ao_app->default_theme, p_miscname);
  start_playback(final_image);
  play();
}
...
void StickerLayer::load_image(QString p_charname)
{
  ...
  transform_mode = ao_app->get_misc_scaling(p_miscname);
  QString final_image = ao_app->get_image("sticker/" + p_charname, options.theme(), options.subTheme(), ao_app->default_theme, p_miscname);
  start_playback(final_image);
  play();
}
...
void AOLayer::start_playback(QString p_image)
{
  ...
  if (!options.continuousPlaybackEnabled()) {
    continuous = false;
    force_continuous = true;
  }

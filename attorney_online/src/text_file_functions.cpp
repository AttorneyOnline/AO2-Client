...
Qt::TransformationMode AOApplication::get_scaling(QString p_scaling)
{
  if (p_scaling.isEmpty())
    p_scaling = options.defaultScalingMode();
...
{
  QString value = get_config_value(p_identifier, p_file, options.theme(), options.subTheme(), default_theme);
  QPoint return_value;
...
QString AOApplication::get_design_element(QString p_identifier, QString p_file,
                                          QString p_misc)
{
  QString value = get_config_value(p_identifier, p_file, options.theme(), options.subTheme(), default_theme, p_misc);
  if (!value.isEmpty())
    return value;
  return "";
}
...
{
  QString path = get_asset(p_file, options.theme(), options.subTheme(), default_theme);
  QFile design_ini;
...
{
  // New Chadly method
  QString value = get_config_value(p_identifier, "chat_config.ini", options.theme(), options.subTheme(), default_theme, p_chat);
  if (!value.isEmpty())
    return value.toUtf8();
...
{
  return get_config_value(p_identifier, "penalty/penalty.ini", options.theme(),
                          options.subTheme(), default_theme, "");
}


QString AOApplication::get_court_sfx(QString p_identifier, QString p_misc)
{
  QString value = get_config_value(p_identifier, "courtroom_sounds.ini", options.theme(), options.subTheme(), default_theme, p_misc);
  if (!value.isEmpty())
    return value.toUtf8();
  return "";
}
...
QStringList AOApplication::get_effects(QString p_char)
{
  const QStringList l_filepath_list{
    get_asset("effects/effects.ini", options.theme(), options.subTheme(), default_theme, ""),
    get_asset("effects.ini", options.theme(), options.subTheme(), default_theme, read_char_ini(p_char, "effects", "Options")),
  };
                                  ...
                                  QString p_folder)
{
  ...
  QStringList paths {
    get_image("effects/" + effect, options.theme(), options.subTheme(), default_theme, ""),
    get_image(effect, options.theme(), options.subTheme(), default_theme, p_folder)
  };
                                           ...
                                           QString p_folder, QString p_property)
{
  ...
  const auto paths = get_asset_paths("effects/effects.ini", options.theme(), options.subTheme(), default_theme, "");
  const auto misc_paths = get_asset_paths("effects.ini", options.theme(), options.subTheme(), default_theme, p_folder);
  QString path;
  QString f_result;
